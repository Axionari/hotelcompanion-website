...
function TopBar({ c, count }: { c: SuiteCopy; count: number }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexShrink: 0 }}>
      <span
        style={{ ...MONO, display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 'clamp(8px,0.72vw,10.5px)', letterSpacing: '.16em', color: TERRA, border: '1px solid rgba(200,106,58,0.4)', background: 'rgba(11,9,8,0.4)', borderRadius: 999, padding: '6px 13px' }}
      >
        <span style={{ width: 6, height: 6, borderRadius: '50%', background: TERRA }} />
        {c.property.toUpperCase()}
      </span>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <span style={{ ...MONO, display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 'clamp(8px,0.7vw,10px)', letterSpacing: '.1em', color: DIM, border: '1px solid rgba(243,236,226,0.14)', borderRadius: 999, padding: '5px 11px' }}>
          <span style={{ width: 5, height: 5, borderRadius: '50%', background: TERRA }} /> EN
        </span>
        <span style={{ ...MONO, fontSize: 'clamp(8px,0.72vw,10.5px)', letterSpacing: '.06em', color: CREAM, border: '1px solid rgba(243,236,226,0.16)', background: 'rgba(243,236,226,0.05)', borderRadius: 999, padding: '5px 12px' }}>
          ▤ {count} · {c.total}
        </span>
      </div>
    </div>
...
function WelcomeScreen({ c }: { c: SuiteCopy }) {
  return (
    <div style={{ ...pad, alignItems: 'center', justifyContent: 'center', textAlign: 'center', gap: 'clamp(10px,1.4vw,20px)' }}>
      ...
      <div style={{ display: 'flex', gap: 'clamp(8px,1vw,14px)', marginTop: 'clamp(8px,1vw,16px)', width: 'min(420px, 80%)' }}>
        ...
        {c.list.map((s) => (
          <div key={s.key} style={{ position: 'relative', display: 'flex', flexDirection: 'column', borderRadius: 16, overflow: 'hidden', background: 'rgba(243,236,226,0.04)', border: `1px solid ${s.featured ? 'rgba(200,106,58,0.6)' : 'rgba(243,236,226,0.1)'}`, boxShadow: s.featured ? '0 0 40px -12px rgba(200,106,58,0.5)' : 'none' }}>
            <div style={{ position: 'relative', flex: 1, minHeight: 'clamp(80px,10vw,180px)' }}>
              ...
              {s.featured && <span style={{ position: 'absolute', top: 10, left: 10, ...MONO, fontSize: 'clamp(6.5px,0.7vw,9px)', letterSpacing: '.08em', color: '#1a1207', background: TERRA, borderRadius: 999, padding: '4px 9px' }}>{c.featured}</span>}
            </div>
            <div style={{ padding: 'clamp(9px,1.1vw,15px)', display: 'flex', flexDirection: 'column', gap: 'clamp(6px,0.7vw,9px)', flexShrink: 0 }}>
              <div>
                <div style={{ fontFamily: SERIF, fontWeight: 530, fontSize: 'clamp(12px,1.15vw,18px)', lineHeight: 1.15, color: CREAM }}>{s.name}</div>
                <div style={{ fontFamily: SANS, fontSize: 'clamp(8.5px,0.85vw,12px)', color: DIM, marginTop: 3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.meta}</div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                <span style={{ fontFamily: SERIF, fontWeight: 530, fontSize: 'clamp(12px,1.15vw,18px)', color: CREAM }}>{s.price}<span style={{ ...MONO, fontSize: 'clamp(7px,0.7vw,10px)', color: 'rgba(242,233,218,0.5)' }}> {c.nightShort}</span></span>
                <span style={{ fontFamily: SANS, fontWeight: 600, fontSize: 'clamp(8.5px,0.85vw,12px)', color: s.featured ? '#1a1207' : CREAM, background: s.featured ? TERRA : 'transparent', border: s.featured ? '1px solid transparent' : '1px solid rgba(243,236,226,0.35)', borderRadius: 999, padding: 'clamp(5px,0.6vw,8px) clamp(11px,1.2vw,16px)' }}>{c.view}</span>
              </div>
            </div>
          </div>
        ))}
    ...
    <div style={pad}>
      ...
      <div style={{ marginTop: 'clamp(12px,1.5vw,20px)', flex: 1, minHeight: 0, display: 'grid', gridTemplateColumns: '1.15fr 1fr', gap: 'clamp(14px,1.8vw,28px)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          ...
          <p style={{ fontFamily: SANS, fontSize: 'clamp(11px,1.05vw,14.5px)', lineHeight: 1.5, color: 'rgba(242,233,218,0.82)', marginTop: 'clamp(8px,1vw,13px)' }}>{d.desc}</p>
          <div style={{ marginTop: 'clamp(9px,1.1vw,15px)', display: 'flex', flexWrap: 'wrap', gap: 'clamp(5px,0.6vw,8px)' }}>
            {d.features.map((f) => <span key={f} style={{ fontFamily: SANS, fontSize: 'clamp(8.5px,0.82vw,11.5px)', color: 'rgba(242,233,218,0.8)', border: '1px solid rgba(243,236,226,0.15)', background: 'rgba(243,236,226,0.04)', borderRadius: 999, padding: 'clamp(5px,0.55vw,7px) clamp(9px,1vw,13px)', whiteSpace: 'nowrap' }}>{f}</span>)}
          </div>
          <div style={{ marginTop: 'clamp(10px,1.2vw,16px)', display: 'flex', alignItems: 'center', gap: 'clamp(10px,1.2vw,15px)', border: '1px solid rgba(243,236,226,0.1)', background: 'rgba(243,236,226,0.03)', borderRadius: 14, padding: 'clamp(9px,1.1vw,15px)' }}>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ ...MONO, fontSize: 'clamp(7.5px,0.7vw,9.5px)', letterSpacing: '.12em', color: TERRA }}>{d.includesLabel}</div>
...
function CartScreen({ c }: { c: SuiteCopy }) {
  return (
    <div style={pad}>
      <TopBar c={c} count={5} />
      <div style={{ textAlign: 'center', marginTop: 'clamp(6px,1vw,14px)', flexShrink: 0 }}>
        <div style={{ ...MONO, fontSize: 'clamp(8px,0.8vw,11px)', letterSpacing: '.18em', color: TERRA }}>{c.cart.label}</div>
        <div style={{ fontFamily: SERIF, fontWeight: 500, fontSize: 'clamp(20px,2.2vw,34px)', color: CREAM, marginTop: 4 }}>{c.cart.title}</div>
      </div>
      <div style={{ marginTop: 'clamp(10px,1.3vw,18px)', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 'clamp(7px,0.9vw,12px)', overflow: 'hidden' }}>
        {c.cart.items.map((it) => (
          <div key={it.name} style={{ display: 'flex', alignItems: 'center', gap: 'clamp(10px,1.2vw,16px)', background: 'rgba(243,236,226,0.04)', border: '1px solid rgba(243,236,226,0.1)', borderRadius: 14, padding: 'clamp(8px,0.9vw,12px)' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontFamily: SANS, fontSize: 'clamp(8.5px,0.85vw,12px)', color: DIM, marginTop: 2 }}>{it.meta}</div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'clamp(6px,0.8vw,11px)', flexShrink: 0 }}>
      ...
      <div style={{ marginTop: 'clamp(8px,1vw,14px)', flexShrink: 0 }}><Cta>{c.cart.reviewCta} · {c.review.subtotal}</Cta></div>
    </div>
  )
}
  ...
  return (
    <div style={pad}>
      ...
      <div style={{ textAlign: 'center', marginTop: 'clamp(4px,0.8vw,12px)', flexShrink: 0 }}>
        <div style={{ ...MONO, fontSize: 'clamp(8px,0.8vw,11px)', letterSpacing: '.18em', color: TERRA }}>{r.label}</div>
        <div style={{ fontFamily: SERIF, fontWeight: 500, fontSize: 'clamp(20px,2.2vw,32px)', color: CREAM, marginTop: 4 }}>{r.title}</div>
      </div>
      <div style={{ marginTop: 'clamp(10px,1.3vw,18px)', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 'clamp(8px,1vw,14px)', flexShrink: 0 }}>
        {r.options.map((o) => (
          <div key={o.title} style={{ textAlign: 'center', borderRadius: 14, padding: 'clamp(10px,1.2vw,16px)', border: `1px solid ${o.active ? 'rgba(200,106,58,0.6)' : 'rgba(243,236,226,0.12)'}`, background: o.active ? 'rgba(200,106,58,0.07)' : 'rgba(243,236,226,0.03)' }}>
            ...
            <div style={{ fontFamily: SANS, fontSize: 'clamp(8.5px,0.85vw,12px)', color: DIM, marginTop: 1 }}>{o.sub}</div>
          </div>
        ))}
      </div>
      <div style={{ marginTop: 'clamp(10px,1.3vw,18px)', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 'clamp(5px,0.7vw,9px)' }}>
        {r.summary.map((s) => (
      ...
      <div style={{ marginTop: 'clamp(8px,1vw,14px)', flexShrink: 0 }}><Cta>{r.cta} →</Cta></div>
    </div>
  )
}
...
function VerifyingScreen({ c }: { c: SuiteCopy }) {
  return (
    <div style={{ ...pad, alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
      ...
      <div style={{ ...MONO, fontSize: 'clamp(8px,0.8vw,11px)', letterSpacing: '.2em', color: TERRA, marginTop: 'clamp(16px,2vw,28px)' }}>{c.verifying.label}</div>
  ...
  return (
    <div style={{ ...pad, alignItems: 'center' }}>
      <div style={{ textAlign: 'center', flexShrink: 0 }}>
        ...
        <div style={{ ...MONO, fontSize: 'clamp(8px,0.8vw,11px)', letterSpacing: '.2em', color: TERRA, marginTop: 'clamp(10px,1.2vw,16px)' }}>{f.label}</div>
      ...
      <div style={{ marginTop: 'clamp(12px,1.5vw,20px)', width: '100%', maxWidth: 620, border: '1px solid rgba(243,236,226,0.1)', background: 'rgba(243,236,226,0.03)', borderRadius: 14, padding: 'clamp(12px,1.4vw,20px)', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ borderTop: '1px solid rgba(243,236,226,0.08)', paddingTop: 'clamp(8px,1vw,12px)', display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
